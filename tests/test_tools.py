...
def test_list_tools_returns_22(authed_oauth, cache):
    """v2.2.8: 22 tools after adding zoom_search_history (deep
    client-side keyword search that bypasses Zoom's 24h native-search
    cap). 4 tools remain unimplementable on User-managed PKCE OAuth:
    ...
    assert "zoom_search_history" in names, "v2.2.8 deep search tool"
...
def test_tool_annotations_split_into_three_groups(authed_oauth, cache):
    """Confirm exact bucket counts: 20 read-only, 1 write, 1 destructive."""
    tools = ZoomTools(authed_oauth, cache).list_tools()
...
@pytest.mark.asyncio
async def test_chat_channels_name_filter(authed_oauth, cache):
    """v2.2.8: replaced broken `starred_only` filter (Zoom's
    /chat/users/me/channels endpoint never exposed a starred field) with
    a client-side `name_filter` substring match."""
    cache.put_channels(
        [
            {"id": "C1", "name": "general", "type": 3},
            {"id": "C2", "name": "devs", "type": 1},
            {"id": "C3", "name": "Engineering Reset", "type": 4},
        ]
    )
    out = await ZoomTools(authed_oauth, cache).call_tool(
        "zoom_chat_channels", {"name_filter": "eng"}
    )
    assert '"count": 1' in out[0]["text"]
    assert "Engineering" in out[0]["text"]


@pytest.mark.asyncio
async def test_chat_channels_no_starred_only_param(authed_oauth, cache):
    """v2.2.8: passing `starred_only` (the now-removed broken param)
    must not blow up — extra args should be ignored gracefully."""
    cache.put_channels(
        [{"id": "C1", "name": "general", "type": 3}]
    )
    out = await ZoomTools(authed_oauth, cache).call_tool(
